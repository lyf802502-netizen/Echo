...
/// </summary>
    ...
    {
        _currentLineTypingFinished = true;
    ...
    private void ResetState()
    {
        ...
        isVoiceEnabled = true;
        _currentLineHasDialogueText = false;
        _currentLineTypingFinished = true;
        lastLine = null;
    }
    ...
    private void PlayCurrentLine()
    {
        ...
        if (!string.IsNullOrEmpty(currentLine.Command))
        {
            ...
            // 这样 bgfade 等动画命令不会与文字打字机同时开始。
            // [2026-08-12] 在背景变化前隐藏说话人框和说话人文本，并且清除对白文本，避免旧文本残留在画面上
            gameplayPanel?.ClearDialogueText();
            gameplayPanel?.HideSpeakerBox();
    ...
    private void AdvanceToNextLine(bool skipAnimations)
    {
        ...
        // 【新增 2026-8-14】检查命令是否阻止自动播放
        if (CommandManager.GetInstance().BlockAdvanceInput)
        {
            // 如果有命令阻止点击，不继续前进
    ...
    private System.Collections.IEnumerator LoadScriptWithProgress(string scriptTaskID)
    {
        ...
        ResetState(); // 清理上一局遗留的背景、角色、BGM、特效等运行状态
    ...
    private IEnumerator ExecuteActionsAndContinue(string actionString)
    {
        ...
        // 这种情况下这里必须立刻退出，避免再次推进或把剧情重播一遍。
        if (externalFlowHandled)
        {
            yield break;
        }

        // 命令行的对白必须等命令完成后再开始打字。
        // 例如 bgfade 使用协程等待淡入淡出动画；在这里更新对白可避免文字与动画同时出现。
    ...
    /// <summary>
    /// 请求：当前命令会自行恢复剧情主流程，命令返回后不要再由 ExecuteActionsAndContinue 接管。
    /// </summary>
    public void RequestExternalFlowHandled()
    {
        _externalFlowHandledRequested = true;
    }

    /// <summary>
    /// 清除“命令已自行接管剧情恢复”的请求。
    /// </summary>
    public void ClearExternalFlowHandledRequest()
    {
        _externalFlowHandledRequested = false;
    }

    /// <summary>
    /// 读取并消费“命令已自行接管剧情恢复”的请求。
    /// </summary>
    public bool ConsumeExternalFlowHandledRequest()
    {
        bool result = _externalFlowHandledRequested;
        _externalFlowHandledRequested = false;
        return result;
    }

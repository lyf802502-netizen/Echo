...
public class GlobalDataManager : BaseManager<GlobalDataManager>
{
    ...
    private void LoadGlobalData()
    {
        ...
        if (File.Exists(path))
        {
            ...
            globalData = JsonMapper.ToObject<GlobalData>(json);
        }
        else
        {
            // 创建默认全局数据
        ...
        if (globalData.UnlockedChapters == null)
        {
            globalData.UnlockedChapters = new List<string>();
        }
        if (globalData.CompletedChapters == null)
        {
            globalData.CompletedChapters = new List<string>();
        }
        ...
        if (globalData.UnlockedChapters.Count == 0)
        {
            globalData.UnlockedChapters.Add("CH01"); // 默认解锁第一个章节
            SaveGlobalData();
        }
    }
    ...
    {
        string path = Application.persistentDataPath + "/" + GLOBAL_DATA_PATH;
        string json = JsonMapper.ToJson(globalData);
    ...
    public void UnlockChapter(string chapterID)
    {
        ...
        if (!globalData.UnlockedChapters.Contains(chapterID))
        {
            globalData.UnlockedChapters.Add(chapterID);
            SaveGlobalData();
            EventCenter.GetInstance().EventTrigger("ChapterUnlocked", chapterID);
        }
    }
    ...
    {
        EnsureInitialized();
        return globalData.UnlockedChapters.Contains(chapterID);
    }
    ...
    public void CompleteChapter(string chapterID)
    {
        ...
        if (!globalData.CompletedChapters.Contains(chapterID))
        {
            globalData.CompletedChapters.Add(chapterID);
            SaveGlobalData();
            EventCenter.GetInstance().EventTrigger("ChapterCompleted", chapterID);
        }
    }
    ...
    {
        EnsureInitialized();
        return globalData.CompletedChapters.Contains(chapterID);
    }
